<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier un patient dans la file d\'attente') }}
        </h2>
    </x-slot>
        ...
        <div class="p-6 text-gray-900">
            <form action="{{ route('file-attente.update', $fileAttente) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="patient_id" id="patient_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('patient_id') border-red-500 @enderror" required>
                        ...
                        @foreach ($patients as $patient)
                            <option value="{{ $patient->id }}" {{ old('patient_id', $fileAttente->patient_id) == $patient->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <select name="consultation_id" id="consultation_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('consultation_id') border-red-500 @enderror" required>
                        ...
                        @foreach ($consultations as $consultation)
                            <option value="{{ $consultation->id }}" {{ old('consultation_id', $fileAttente->consultation_id) == $consultation->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="datetime-local" name="heure_arrivee" id="heure_arrivee" value="{{ old('heure_arrivee', $fileAttente->heure_arrivee->format('Y-m-d\TH:i')) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('heure_arrivee') border-red-500 @enderror" required>
                    @error('heure_arrivee')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="position" id="position" value="{{ old('position', $fileAttente->position) }}" min="1" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('position') border-red-500 @enderror" required>
                    @error('position')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="statut" id="statut" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('statut') border-red-500 @enderror" required>
                        <option value="en attente" {{ old('statut', $fileAttente->statut) == 'en attente' ? 'selected' : '' }}>En attente</option>
                        <option value="en consultation" {{ old('statut', $fileAttente->statut) == 'en consultation' ? 'selected' : '' }}>En consultation</option>
                        <option value="terminé" {{ old('statut', $fileAttente->statut) == 'terminé' ? 'selected' : '' }}>Terminé</option>
                        <option value="absent" {{ old('statut', $fileAttente->statut) == 'absent' ? 'selected' : '' }}>Absent</option>
                    </select>
                    @error('statut')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
