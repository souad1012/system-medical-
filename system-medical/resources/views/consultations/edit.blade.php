        ...
        <div class="p-6 text-gray-900">
            <form action="{{ route('consultations.update', $consultation) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="salle_id" id="salle_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('salle_id') border-red-500 @enderror" required>
                        ...
                        @foreach ($salles as $salle)
                            <option value="{{ $salle->id }}" {{ old('salle_id', $consultation->salle_id) == $salle->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="datetime-local" name="heure_debut" id="heure_debut" value="{{ old('heure_debut', $consultation->heure_debut->format('Y-m-d\TH:i')) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('heure_debut') border-red-500 @enderror" required>
                    @error('heure_debut')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="datetime-local" name="heure_fin" id="heure_fin" value="{{ old('heure_fin', $consultation->heure_fin ? $consultation->heure_fin->format('Y-m-d\TH:i') : '') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('heure_fin') border-red-500 @enderror">
                    @error('heure_fin')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="type" id="type" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('type') border-red-500 @enderror" required>
                        ...
                        <option value="Première visite" {{ old('type', $consultation->type) == 'Première visite' ? 'selected' : '' }}>Première visite</option>
                        <option value="Suivi" {{ old('type', $consultation->type) == 'Suivi' ? 'selected' : '' }}>Suivi</option>
                        <option value="Urgence" {{ old('type', $consultation->type) == 'Urgence' ? 'selected' : '' }}>Urgence</option>
                        <option value="Spécialiste" {{ old('type', $consultation->type) == 'Spécialiste' ? 'selected' : '' }}>Spécialiste</option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <textarea name="notes" id="notes" rows="4" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('notes') border-red-500 @enderror">{{ old('notes', $consultation->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="statut" id="statut" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('statut') border-red-500 @enderror" required>
                        <option value="planifiée" {{ old('statut', $consultation->statut) == 'planifiée' ? 'selected' : '' }}>Planifiée</option>
                        <option value="en cours" {{ old('statut', $consultation->statut) == 'en cours' ? 'selected' : '' }}>En cours</option>
                        <option value="terminée" {{ old('statut', $consultation->statut) == 'terminée' ? 'selected' : '' }}>Terminée</option>
                        <option value="annulée" {{ old('statut', $consultation->statut) == 'annulée' ? 'selected' : '' }}>Annulée</option>
                    </select>
                    @error('statut')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
